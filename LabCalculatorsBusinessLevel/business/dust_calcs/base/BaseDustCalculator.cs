...
abstract class BaseDustCalculator : FormulaTypeCalculator
{
    ...
    abstract protected double Error_index { get; } // Коэффициент погрешности.

    public BaseDustCalculator(DustCalcsParameters parameters_object) : base(parameters_object)
    {
        var mass_concentration = GetMassConcentration();

        Result_data.Add(mass_concentration);
        Result_data.Add(GetResultError(mass_concentration));
    }
    ...
    double GetResultError(double mass_concentrate)
    {
